...
Envelope& Envelope::header(ActorRef& rcv, ActorRef& snd, MsgClass clz, uint16_t i)
{
    sender = &snd;
    receiver = &rcv;
    msgClass = clz;
...
{
    return s.format(" dst : %s , src : %s , class : %s  ", receiver->path(), sender->path(), msgClass.label());
}
...
Mailbox::Mailbox(const char* name, uint32_t queueSize, uint32_t messageSize)
    : MessageQueue(queueSize, messageSize)
    , _name(name)
{
    _mailboxes.add(this);
...
//
Receiver::Receiver(MsgClass msgClass, MessageMatcher matcher, MessageHandler handler)
    : _msgClass(msgClass)
    , _matcher(matcher)
    , _handler(handler)
{
...
uid_type TimerScheduler::startPeriodicTimer(UidType key, MsgClass cls, uint32_t msec)
{
...
uid_type TimerScheduler::startSingleTimer(UidType key, MsgClass, uint32_t msec)
{
    Timer* timer = find(key.id());
...
//
ActorCell::ActorCell(ActorSystem& system, ActorRef& ref, Mailbox& mailbox, MessageDispatcher& dispatcher)
    : _mailbox(mailbox)
    , _system(system)
